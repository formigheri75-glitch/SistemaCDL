...
const FilterSection = {
    props: {
        label: { type: String, default: 'Filtrar:' },
        options: { type: Array, required: true },
        activeValue: { type: String, default: null }
    },
    emits: ['update:activeValue'],
    template: `
    <div class="filter-section">
        ...
        <button
            ...
            :key="opt.value"
            class="filter-button"
            :class="{ active: activeValue === opt.value }"
            @click="$emit('update:activeValue', opt.value)"
        >{{ opt.label }}{{ opt.count !== undefined ? ' (' + opt.count + ')' : '' }}</button>
    </div>
    `
};
...
const EmptyState = {
    props: {
        message: { type: String, default: 'Nenhum item encontrado.' }
    },
    template: '<div class="empty-state"><p>{{ message }}</p></div>'
};
...
const StatusBadge = {
    ...
    computed: {
        cssClass() {
            return window.classStatusProposta(this.status);
        },
        label() {
            return window.getStatusLabel(this.status) || this.status || 'Pendente';
        }
    },
    template: '<span class="status-pill" :class="cssClass">{{ label }}</span>'
};
...
const FormAlert = {
    ...
    template: '<div v-if="message" :class="type === \'error\' ? \'form-error\' : \'form-success\'">{{ message }}</div>'
};
...
window.formatarData = function(raw, fallback) {
...
window.getLoggedEmpresaId = async function() {
    try {
        ...
        if (user.sourceTable === 'empresas' && user.id) return user.id;
        const empresas = await window.ApiClient.get('/empresas');
        const found = (empresas || []).find(e =>
            String(e.id || e.ID) === String(user.id) ||
...
window.getLoggedInstituicaoId = async function() {
    try {
        const user = window.StorageManager.getUser() || {};
...
window.getUserRole = function() {
    ...
    const role = user && (user.tipo || user.type || user.role || user.tipousuario) || localStorage.getItem('userType') || 'instituicao';
